...
const ProductTypeSettings: React.FC = () => {
  ...
  const handleAddProductType = async (e: React.FormEvent) => {
    ...
    try {
      if (editingType) {
        ...
        await updateDoc(doc(db, 'productTypes', editingType.id), {
          name: formData.name,
          description: formData.description,
          // Keep the default status when editing
          ...(editingType.default !== undefined && { default: editingType.default })
        });
        ...
        const docRef = await addDoc(collection(db, 'productTypes'), newProductType);
        ...
        setProductTypes([...productTypes, { id: docRef.id, ...newProductType }]);
  ...
  const handleDelete = async () => {
    if (!typeToDelete || !currentUser) return;
    ...
    try {
      ...
      setProductTypes(productTypes.filter(type => type.id !== typeToDelete.id));
  ...
  // New function to handle setting a default product type
  const handleSetDefault = async (typeId: string) => {
    if (!currentUser) return;
    
    try {
      // Find the product type in the state
      const productType = productTypes.find(type => type.id === typeId);
      if (!productType) return;
      
      // If the product type is already the default, do nothing
      if (productType.default) return;
      
      // First, remove default flag from any existing default product type
      const defaultProductType = productTypes.find(type => type.default === true);
      if (defaultProductType) {
        await updateDoc(doc(db, 'productTypes', defaultProductType.id), {
          default: false
        });
      }
      
      // Set this product type as the new default
      await updateDoc(doc(db, 'productTypes', typeId), {
        default: true
      });
      ...
      console.error('Error setting default product type:', error);
    }
  };
    ...
    <form onSubmit={handleAddProductType}>
      <div className="grid grid-cols-1 gap-6">
        ...
        <div className="flex justify-end space-x-3">
          ...
          >
            {isEdit ? 'Update Product Type' : 'Add Product Type'}
          </button>
        </div>
      </div>
    </form>
        ...
        <div className="sm:flex sm:items-start">
          ...
          <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            <p className="text-sm text-gray-500">
              Are you sure you want to delete {typeToDelete?.name}? This may affect products assigned to this type.
            </p>
          </div>
        </div>
